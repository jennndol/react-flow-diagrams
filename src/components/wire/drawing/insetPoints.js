...
export const insetPoints = (rawPoints, rawIntersections) => {
  ...
  if (Math.abs(startSegment.length) > startInset) {
    ...
    points[0] = startSegment.h ? [points[0][0] + delta, points[0][1]] : [points[0][0], points[0][1] + delta];
  } else {
    points.splice(0, 1);
  ...
  if (Math.abs(endSegment.length) > minimumEndSegmentLength) {
    ...
    points[l - 1] = endSegment.h ? [points[l - 1][0] + delta, points[l - 1][1]] : [points[l - 1][0], points[l - 1][1] + delta];
  } else {
    points.splice(l - 1, 1);
