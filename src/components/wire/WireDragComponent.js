...
export default class WireDragComponent extends PureComponent {
  ...
  render() {
    const idMatch = this.props.dragStatus && this.props.dragStatus.id === this.props.wire.id;
    const startDragged = idMatch && this.props.dragStatus.status === 'wire-start';
    const endDragged = idMatch && this.props.dragStatus.status === 'wire-end';
    const startGrabberClass = 'wire-outer-grabber grabber' + (startDragged ? ' dragged' : '');
    const endGrabberClass = 'wire-outer-grabber grabber' + (endDragged ? ' dragged' : '');
    const start = this.props.wire.points[0];
    const end = this.props.wire.points[this.props.wire.points.length - 1];

    const startStyle = {
      visibility: this.props.wire.selected && this.props.wire.startNodeId === null ? 'visible' : 'hidden',
      left: start[0] + 'px',
      top: start[1] + 'px',
    };
    const endStyle = {
      visibility: this.props.wire.selected && this.props.wire.endNodeId === null ? 'visible' : 'hidden',
      left: end[0] + 'px',
      top: end[1] + 'px',
    };
    ...
    return (
      <div className='wire-resizer-group' ref={e => this.root = e}>
        ...
        <div className={startGrabberClass} ref={e => this.startResizer = e} style={startStyle}>
          <div className='wire-outer-resizer' />
        </div>
        <div className={endGrabberClass} ref={e => this.endResizer = e} style={endStyle}>
          <div className='wire-outer-resizer' />
        </div>
      </div>
    );
  ...
  createInnerResizer(r) {
    const dragged = this.props.dragStatus
    ...
    const key = r.i + ((dragged && this.props.wire.extensionInProgress) ? '-dragged-extended' : '');
    const className = 'wire-inner-grabber grabber' + (dragged ? ' dragged' : '');
    return (
      <div key={key} className={className} style={style} ref={e => this.addResizeListener(e, r)}>
  ...
  onEndMouseDown = event => {
    ...
    if (event.button === 0 && !this.props.dragStatus) {
      ...
      this.mouseXAtPress = event.clientX;
      this.mouseYAtPress = event.clientY;
      this.endAtPress = this.props.wire.points[this.props.wire.points.length - 1];
      this.pointsAtPress = this.props.wire.points;
  ...
  onMouseMove = event => {
    if (this.endDragInProgress) {
      ...
      const newPoints = movePointRectangular(this.props.wire.points, i, h, newX, newY);
      const axis = hDragAllowed && vDragAllowed ? 'both' : hDragAllowed ? 'h' : 'v';
      const snapInput = { i: this.draggedPoint.i, h: this.draggedPoint.h, axis };
  ...
  getCursor(index, a, b, c) {
    ...
    if (index === 1) {
      ...
      return firstIncreasing === secondIncreasing ? 'nesw-resize' : 'nwse-resize';
    }
  }
