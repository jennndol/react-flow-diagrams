...
export default class WireTextComponent extends PureComponent {
  ...
  render() {
    ...
    const className = 'wire-text grabber' + (selected ? ' selected' : '') + (this.props.wire.textEditable ? ' editable' : '');
    const hasText = this.props.wire.editorState && this.props.wire.editorState.getCurrentContent().hasText();
    const style = {
      left: position[0] + 'px',
      top: position[1] + 'px',
