    ...
    return (
      <div className='wire-layer'>
        <svg className='wire-path-layer'>
          {this.props.wires.map((w, i) => <WirePathContainer key={i} wire={w} />)}
        </svg>
        <svg className='wire-path-head-layer'>
          {this.props.wires.map((w, i) => <WireHeadComponent key={i} wire={w} />)}
        </svg>
        <div className='wire-drag-layer'>
          {this.props.wires.map((w, i) => <WireDragContainer key={i} wire={w} />)}
        </div>
        <div className='wire-text-layer'>
          {this.props.wires.map((w, i) => <WireTextContainer key={i} wire={w} />)}
        </div>
      </div >
    );
  }
