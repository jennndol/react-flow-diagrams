...
export default class WireExtenderComponent extends PureComponent {
  ...
  render() {
    const visible = !this.props.wire.extensionInProgress && this.isSnapPointShowing() && !this.props.anyTextEdited;
    const className = 'wire-extender' + (visible ? ' showing' : '');
    const style = { visibility: this.props.wire.extensionInProgress ? 'hidden' : 'visible' };
    ...
    return <path className={className} style={style} d={d} ref={e => this.rect = e} />;
  }
  ...
  onMouseDown = event => {
    if (event.button === 0 && this.isSnapPointShowing() && !this.props.wire.extensionInProgress) {
      ...
      this.draggedPoint = getPointToDrag(this.props.wire.points, i - 1, newPoint);
  ...
  isSnapPointShowing = () => this.state.snapPoint !== null && this.props.wireCreationOn && this.state.pathHover;
