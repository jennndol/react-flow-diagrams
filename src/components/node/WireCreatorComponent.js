...
export default class WireCreatorComponent extends Component {
  ...
  state = { side: null, snappedToTarget: false, localX: 0, localY: 0, nodeHover: false, suppressed: false };
  lastVisibleX = 0;
  lastVisibleY = 0;
  createdWireId = null;
  xAtPress = 0;
  ...
  shouldComponentUpdate(nextProps, nextState) {
    ...
    const visibilityChanged = this.isVisible(this.state, this.props) !== nextVisible;
    return nextVisible || visibilityChanged;
  }

  componentWillReceiveProps(nextProps) {
    if (!this.props.wireCreationOn && nextProps.wireCreationOn && this.clientX && this.clientY) {
      this.checkForSnap();
    }
  }
  ...
  render() {
    ...
    const style = {
      ...
      visibility: (this.createInProgress || this.props.anyTextEdited) ? 'hidden' : 'visible',
    }
    if (visible) {
    ...
    return (
      <div className='wire-creator' style={style} ref={e => this.element = e}>
        <svg className='wire-creator-plus-svg' >
          <path className='wire-creator-plus' d='M 4 0 h 3 v 4 h 4 v 3 h -4 v 4 h -3 v -4 h -4 v -3 h 4 Z' />
        </svg>
  ...
  onMouseMove = event => {
    ...
    if (this.createInProgress) {
      if (!this.wireWasCreated) {
        ...
        this.props.onWireCreationStart(this.getStartPos(), this.getMousePositionInGraph(event), this.state.side);
        this.createdWireId = Math.max(...this.props.node.startConnectors.map(c => c.id));
        this.props.onWireCreated(this.createdWireId);
      } else if (this.createdWireId !== null) {
        if (!this.thresholdExceeded) {
          const dragDelta = Math.hypot(event.clientX - this.xAtPress, event.clientY - this.yAtPress);
          this.thresholdExceeded = dragDelta > thresholdToAllowSelfConnect;
        }
        this.props.onCreatedWireDrag(this.createdWireId, this.getMousePositionInGraph(event), this.thresholdExceeded);
      }
    ...
    if (this.mouseDown && !this.state.suppressed) {
      ...
      const deltaSinceRelease = Math.hypot(event.clientX - this.xAtRelease, event.clientY - this.yAtRelease);
  ...
  isVisible = (state, props) => state.side !== null
  ...
  checkForSnap = () => {
    ...
    const snapResult = snapToBox(mousePositionInGraph[0], mousePositionInGraph[1], this.props.node);
    ...
    const needsUpdate = this.state.side !== side
