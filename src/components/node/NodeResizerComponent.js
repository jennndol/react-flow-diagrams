...
export default class NodeResizerComponent extends PureComponent {
  ...
  render() {
    return (
      <div className='resizer-group' ref={e => this.root = e}>
        <div className={this.getClasses('north-west')} ref={e => this.northWest = e}>
          <div className='resizer' />
        </div>
        <div className={this.getClasses('north')} ref={e => this.north = e}>
          <div className='resizer' />
        </div>
        <div className={this.getClasses('north-east')} ref={e => this.northEast = e}>
          <div className='resizer' />
        </div>
        <div className={this.getClasses('east')} ref={e => this.east = e}>
          <div className='resizer' />
        </div>
        <div className={this.getClasses('south-east')} ref={e => this.southEast = e}>
          <div className='resizer' />
        </div>
        <div className={this.getClasses('south')} ref={e => this.south = e}>
          <div className='resizer' />
        </div>
        <div className={this.getClasses('south-west')} ref={e => this.southWest = e}>
          <div className='resizer' />
        </div>
  ...
  getClasses = direction => 'resizer-grabber '
    + direction
    + (this.props.node.selected ? ' grabber' : '')
    + (this.state.resizeInProgress === direction ? ' dragged' : '');

  onNorthWestDown = event => this.onMouseDown(event, 'north-west', 'nwse-resize');

  onNorthDown = event => this.onMouseDown(event, 'north', 'ns-resize');

  onNorthEastDown = event => this.onMouseDown(event, 'north-east', 'nesw-resize');

  onEastDown = event => this.onMouseDown(event, 'east', 'ew-resize');

  onSouthEastDown = event => this.onMouseDown(event, 'south-east', 'nwse-resize');

  onSouthDown = event => this.onMouseDown(event, 'south', 'ns-resize');

  onSouthWestDown = event => this.onMouseDown(event, 'south-west', 'nesw-resize');
  ...
  onMouseMove = event => {
    if (this.resizeDirections) {
      ...
      this.resizeDirections.forEach(direction => this.doResize(direction, event.clientX, event.clientY, position));
      this.props.onResize(position.x, position.y, position.w, position.h, this.resizeDirections);
  ...
  doResize = (direction, mouseX, mouseY, position) => {
    ...
    if (direction === 'east') {
      position.w = this.clamp(this.widthAtPress + deltaX, minWidth, maxX - this.xAtPress);
    } else if (direction === 'south') {
      position.h = this.clamp(this.heightAtPress + deltaY, minHeight, maxY - this.yAtPress);
    } else if (direction === 'west') {
      position.x = this.clamp(this.xAtPress + deltaX, padding, this.xAtPress + this.widthAtPress - minWidth);
      position.w = this.xAtPress + this.widthAtPress - position.x;
    } else if (direction === 'north') {
      position.y = this.clamp(this.yAtPress + deltaY, padding, this.yAtPress + this.heightAtPress - minHeight);
