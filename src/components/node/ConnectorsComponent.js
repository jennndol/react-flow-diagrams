...
import { getRelativePointOnNode, getAbsolutePointOnNode } from '../../reducers/helpers/getPointOnNode';
...
export default class ConnectorsComponent extends PureComponent {
  ...
  render() {
    return (
      <div ref={e => this.root = e}>
        {this.props.node.startConnectors.map(c => this.createConnector(c, true))}
  ...
  createConnector(connector, start) {
    ...
    const point = getRelativePointOnNode(this.props.node, connector.pos, connector.side);
    const style = {
      left: point[0] + 'px',
      top: point[1] + 'px',
    ...
    const dragged = (this.props.dragStatus && this.props.dragStatus.id === connector.id)
    ...
    return (
      <div className={className} key={key} style={style} ref={e => this.addMouseDownListener(e, key, connector, start)}>
  ...
  addMouseDownListener(element, key, connector, start) {
    ...
    if (element) {
      const mouseDownListener = event => this.onConnectorMouseDown(event, connector, start);
      const clickListener = event => event.stopPropagation();

      this.addListener(this.mouseDownListeners, element, mouseDownListener, key, 'mousedown');
      this.addListener(this.clickListeners, element, clickListener, key, 'click');
    }
  }
  ...
  onConnectorMouseDown = (event, connector, start) => {
    ...
    if (event.button === 0) {
      ...
      this.initialPosition = getAbsolutePointOnNode(this.props.node, connector.pos, connector.side);
  ...
  onMouseMove = event => {
    if (this.draggedConnector) {
      ...
      this.props.onConnectorDrag(this.draggedConnector.id, [x, y], this.startDragged);
    }
  }
  ...
  endDrag = () => {
    if (this.draggedConnector) {
      this.props.onConnectorDragEnd(this.draggedConnector.id, this.startDragged, this.didConnectorMove());
  ...
  didConnectorMove() {
    const newConnectors = this.startDragged ? this.props.node.startConnectors : this.props.node.endConnectors;
    if (newConnectors.some(c => c.id === this.draggedConnector.id)) {
      const newConnector = newConnectors.filter(c => c.id === this.draggedConnector.id)[0];
      return this.draggedConnector.pos !== newConnector.pos || this.draggedConnector.side !== newConnector.side;
    } else {
