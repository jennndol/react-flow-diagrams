...
export default class NodeComponent extends PureComponent {
  ...
  onMouseMove = event => {
    if (this.dragInProgress) {
      ...
      if (!this.thresholdExceeded) {
        this.thresholdExceeded = Math.abs(deltaX) > dragThreshold || Math.abs(deltaY) > dragThreshold;
      }
      if (this.thresholdExceeded) {
  ...
  onDoubleClick = event => {
    if (event.button === 0 && !this.props.node.textEditable && !event.ctrlKey && !event.metaKey) {
