...
function updateEditable(element, id, textEditable) {
  if (element.id === id && element.textEditable !== textEditable) {
    let editorState = element.editorState ? element.editorState : EditorState.createEmpty();
    editorState = EditorState.moveFocusToEnd(EditorState.moveSelectionToEnd(editorState));
