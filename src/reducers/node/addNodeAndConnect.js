...
export const enoughSpaceToAdd = (nodes, id, direction) => {
  ...
  if (direction === 'east') {
    return sourceNode.x + 2 * sourceNode.width + offset < graph.clientWidth - padding;
  } else if (direction === 'west') {
    return sourceNode.x - offset - sourceNode.width > padding;
  } else if (direction === 'south') {
    return sourceNode.y + 2 * sourceNode.height + offset < graph.clientHeight - padding;
  } else {
...
export const addNodeAndConnect = (state, id, direction) => {
  ...
  const newNode = createNode(sourceNode.x, sourceNode.y, sourceNode.width, sourceNode.height, true);

  configureSize(newNode, wire, direction);

  const wireStartPoint = getAbsolutePointOnNode(sourceNode, 0.5, wire.startSide);
  const wireEndPoint = getAbsolutePointOnNode(newNode, 0.5, wire.endSide);
  wire.points = layoutRectangularPath(wireStartPoint, wireEndPoint, sourceNode, newNode, wire.startSide, wire.endSide);
