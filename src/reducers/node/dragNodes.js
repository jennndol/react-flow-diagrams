...
export const dragNodes = (nodes, snapAllowed, draggedNodeId, dX, dY) => {
  ...
  const nodeAfterDrag = { ...nodes.filter(n => n.id === draggedNodeId)[0], x: draggedNodeX, y: draggedNodeY };
  const stationaryNodes = nodes.filter(n => !n.selected && n.id !== draggedNodeId);
  const nodeAfterSnap = snapAllowed ? snapIfInRange(nodeAfterDrag, stationaryNodes) : nodeAfterDrag;
  const snappedDeltaX = nodeAfterSnap.x - nodePositionsAtDragStart[draggedNodeId].x;
  const snappedDeltaY = nodeAfterSnap.y - nodePositionsAtDragStart[draggedNodeId].y;
  const clampedDeltaX = clamp(snappedDeltaX, dragBounds.minDeltaX, dragBounds.maxDeltaX);
  const clampedDeltaY = clamp(snappedDeltaY, dragBounds.minDeltaY, dragBounds.maxDeltaY);

  return nodes.map(n => dragNode(n, clampedDeltaX, clampedDeltaY));
}

export const resizeNode = (node, snapAllowed, otherNodes, action) => {
  const nodeAfterResize = { ...node, x: action.x, y: action.y, width: action.width, height: action.height };
  const nodeAfterSnap = snapAllowed ? snapIfInRange(nodeAfterResize, otherNodes, action.directions) : nodeAfterResize;
