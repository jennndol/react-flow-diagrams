...
export const findIntersections = wires => {
  ...
  intersections.forEach((w, i) => w.forEach((s, j) => sortIntersections(s, wires[i].points, j)));
...
function queryWireIntersections(wire, otherWire, queriesForWire, wireIndex, otherWireIndex) {
    ...
    for (let j = 0; j < jMax; j++) {
      ...
      if (abh !== cdh) {
        if (abh && intersects(a, b, c, d, aInset, bInset, cInset, dInset)) {
          queriesForWire[i].push({ p: [c[0], a[1]], h: true, otherWireIndex, otherSegmentIndex: j, added: false });

        } else if (cdh && intersects(c, d, a, b, cInset, dInset, aInset, bInset)) {
          queriesForWire[i].push({ p: [a[0], c[1]], h: false, otherWireIndex, otherSegmentIndex: j, added: false });
        }
      }
    }
  }
...
function intersects(a, b, c, d, aInset, bInset, cInset, dInset) {
  return intersectsHorizontal(a, b, c, d, aInset, bInset) && intersectsVertical(a, b, c, d, cInset, dInset);
}

function intersectsHorizontal(a, b, c, d, aInset, bInset) {
  return (c[0] > a[0] + aInset && c[0] < b[0] - bInset) || (c[0] > b[0] + bInset && c[0] < a[0] - aInset);
}

function intersectsVertical(a, b, c, d, cInset, dInset) {
  return (a[1] > c[1] + cInset && a[1] < d[1] - dInset) || (a[1] > d[1] + dInset && a[1] < c[1] - cInset);
}
...
function createIntersectionsAndCheckForMismatch(queries) {
  ...
  queries.forEach((w, i) => w.forEach((s, j) => s.forEach(q => addIfVertical(intersections, q, i, j))));
  return intersections;
}

function addAllHorizontal(intersections, queries) {
  let nothingAdded = true;
  queries.forEach((w, i) => w.forEach((s, j) => s.forEach(q => {
...
function addIfHorizontal(intersections, query, queries, i, j) {
  if (!query.added && !query.h && isTooCloseToHorizontalIntersection(query.p, queries)) {
    query.h = true;
    query.added = true;
    intersections[query.otherWireIndex][query.otherSegmentIndex].push(query.p[0]);
    return true;
...
function isTooCloseToHorizontalIntersection(p, queries) {
  return queries.some(w => w.some(s => s.some(query => query.h && isTooClose(p, query.p))));
}
