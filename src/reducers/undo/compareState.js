...
export const didStateChange = (oldState, newState) => {
  return didNodesChange(oldState.nodes, newState.nodes) || didWiresChange(oldState.wires, newState.wires);
}
...
function didNodeChange(oldNode, newNode) {
  return oldNode.id !== newNode.id
    ...
    || oldNode.startConnectors.some((c, i) => didConnectorChange(c, newNode.startConnectors[i]))
    || oldNode.endConnectors.some((c, i) => didConnectorChange(c, newNode.endConnectors[i]))
    || didTextChange(oldNode.editorState, newNode.editorState);
}

function didConnectorChange(oldConnector, newConnector) {
  return oldConnector.id !== newConnector.id
    || oldConnector.pos !== newConnector.pos
    || oldConnector.side !== newConnector.side;
}

function didWireChange(oldWire, newWire) {
  return oldWire.id !== newWire.id
