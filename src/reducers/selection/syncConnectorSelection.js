export const syncConnectorSelection = (nodes, oldWires, newWires) => {
  for (let wire of newWires) {
    ...
    if (wireChanged && wire.endNodeId !== null && wire.endNodeId !== wire.startNodeId) {
...
function syncConnectorSelectionForNode(nodes, nodeId, wire) {
  ...
  nodes[index].startConnectors.filter(c => c.id === wire.id).forEach(c => c.selected = wire.selected);
  nodes[index].endConnectors.filter(c => c.id === wire.id).forEach(c => c.selected = wire.selected);
}