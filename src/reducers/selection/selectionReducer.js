...
function deleteSelection(state) {
  ...
  const nodes = nodesNotSelected.map(n => cleanConnectorsOfDeletedWires(n, wireIds));
...
function cleanConnectorsOfDeletedWires(node, wireIds) {
  ...
  if (cleanStart || cleanEnd) {
    const newNode = { ...node };
    if (cleanStart) {
      newNode.startConnectors = node.startConnectors.filter(c => wireIds.includes(c.id));
    }
    if (cleanEnd) {
      newNode.endConnectors = node.endConnectors.filter(c => wireIds.includes(c.id));
    }
    return newNode;
  } else {
    return node;
...
function cleanWireOfDeletedNodes(wire, nodeIds) {
  const cleanStart = wire.startNodeId !== null && !nodeIds.includes(wire.startNodeId);
