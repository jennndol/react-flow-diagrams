...
export const addWire = (state, action) => {
  ...
  const newWire = createWireAndConnect(newStartNode, action.startPos, action.point, action.startSide);
  const newWires = [...state.wires, newWire];
  const newNodes = state.nodes.map(n => n.id === startNode.id ? newStartNode : n);
...
function createWireAndConnect(startNode, startPos, point, startSide) {
  ...
  wire.points = layoutRectangularPath(start, point, startNode, null, startSide, null);
